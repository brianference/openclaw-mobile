...
- Shows task board with local persistence
...
- Does NOT actually sync with web task board (stub implementation)
...
- [PASS] Task Board cards bound to single board
...
| CG-04 | Task Board syncs with web | Sync is stubbed, doesn't work | HIGH | Implement or remove claim |
...
| S-03 Task Board | Tab 2 | Task management | Cards, priorities | LOW | LOW |
...
| G-06 | Task Board | Long-press card | Opens move menu | May not be discoverable | LOW | Add hint text |
...
**FIX-M05: Implement or remove task board sync**
- Location: `src/store/task board.ts` lines 105-138
...
**FIX-M11: Add long-press hint to task board cards**
- Location: `app/(tabs)/task board.tsx`
...
3. Implement or remove task board sync claims
