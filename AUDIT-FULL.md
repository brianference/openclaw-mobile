...
| AsyncStorage | Device | None | No | Messages, task board, token | Device access → full data |
...
| Task Board | User interaction | Card data | Board state | AsyncStorage, Zustand | LOW |
...
| Task Board | Tab 2 | Manage tasks | Cards | None significant |
...
| Empty task board | Empty state | "No cards" per column | NO ✓ | - |
...
| Task Board cards | Created/updated dates hidden | Show in card detail |
...
| Task Board sync stubbed | HIGH | Implement or remove |
...
- Unlimited task board cards (local storage limits)
...
| Task Board | Single thread | AsyncStorage | N/A |
...
| 100+ task board cards | Memory grows | Add lazy loading |
...
| Task Board cards | AsyncStorage | `card_*` | User | None | None |
...
| Save task board | No | Yes (ID-based) | LOW |
...
| Task Board cards | By array order | N/A | No pagination |
...
| FIX-M05 | Implement or remove task board sync | Mobile |
