...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!profile || profile.credits < 1) {
      return new Response(
        JSON.stringify({ error: "Insufficient credits", credits: profile?.credits ?? 0 }),
    ...
    if (!messages || !conversationId) {
      return new Response(
        JSON.stringify({ error: "Missing messages or conversationId" }),
...
async function generateResponse(
  ...
  if (input.includes("weather") || input.includes("time") || input.includes("date")) {
